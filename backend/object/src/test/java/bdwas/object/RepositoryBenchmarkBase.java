...
public abstract class RepositoryBenchmarkBase<R extends JpaRepository<E, I>, E, I> {
    ...
    public void setup() {
        ...
        clearTables();
        existingEntities = generateEntities(recordCount);
        existingEntities.forEach(entity -> entityManager.persist(entity));
    ...
    public void benchmarkDelete() {
        entityManager.getTransaction().begin();
        existingEntities.forEach(entity -> entityManager.remove(entity));
        entityManager.getTransaction().commit();
    }
